...
import Dag from '../index.tsx';
import { mockData, mockData2, mockData4, mockTree_D1N2_D2N2 } from './mockData';

describe('<Dag />', () => {
  jest.spyOn(Dag.prototype, 'generateTree');
  jest.spyOn(Dag, 'assignInternalProperties');
  jest.spyOn(Dag, 'collapseNode');
  jest.spyOn(Dag, 'expandNode');
  // jest.spyOn(Dag.prototype, 'setInitialTreeDepth');
  jest.spyOn(Dag.prototype, 'bindZoomListener');
  jest.spyOn(Dag.prototype, 'collapseNeighborNodes');

  // Clear method spies on prototype after each test
  afterEach(() => jest.clearAllMocks());

  it('builds a tree on each render', () => {
    const renderedComponent = shallow(<Dag data={mockData} />);
  ...
  it('maps every parent-child relation onto a <Link /> with expected siblings', () => {
    ...
    const renderedComponent = shallow(<Dag data={mockData4} />);
  ...
  it("assigns unique ref classNames to each Dag instance's `svg` and primary `g` tag", () => {
    const tree1 = shallow(<Dag data={mockData} />);
    const tree2 = shallow(<Dag data={mockData} />);
    expect(tree1.find('.rd3dag-svg').prop('className')).not.toBe(
      tree2.find('.rd3dag-svg').prop('className')
    );
    expect(tree1.find('.rd3dag-g').prop('className')).not.toBe(
      tree2.find('.rd3dag-g').prop('className')
    );
  });
  ...
  it('reassigns internal props if `props.data` changes', () => {
    ...
    expect(Dag.assignInternalProperties).toHaveBeenCalledTimes(mockDataDepth);
    renderedComponent.setProps(nextProps);
    expect(Dag.assignInternalProperties).toHaveBeenCalledTimes(mockDataDepth + mockData2Depth);
  });
  ...
  it("reassigns internal props if `props.data`'s array reference changes", () => {
    ...
    expect(Dag.assignInternalProperties).toHaveBeenCalledTimes(mockDataDepth + nextDataDepth);
  });
  ...
  describe('orientation', () => {
    it('passes `props.orientation` to its <Node /> and <Link /> children', () => {
      ...
      const renderedComponent = shallow(<Dag data={mockData} orientation={fixture} />);
  ...
  describe('collapsible', () => {
    it("collapses a node's children when it is clicked in an expanded state", () => {
      const renderedComponent = mount(<Dag data={mockData4} />);
  ...
  describe('initialDepth', () => {
    it('expands dag to full depth by default', () => {
      const renderedComponent = shallow(<Dag data={mockTree_D1N2_D2N2} />);
      expect(renderedComponent.find(Node).length).toBe(5);
    });

    it.skip('expands dag to `props.initialDepth` if specified', () => {
      const renderedComponent = shallow(<Dag data={mockTree_D1N2_D2N2} initialDepth={1} />);
      expect(renderedComponent.find(Node).length).toBe(3);
    });

    it.skip('renders only the root node if `initialDepth === 0`', () => {
      const renderedComponent = shallow(<Dag data={mockTree_D1N2_D2N2} initialDepth={0} />);
  ...
  describe('zoom', () => {
    it('applies the `zoom` prop when specified', () => {
      ...
      const renderedComponent = shallow(<Dag data={mockData} zoom={zoomLevel} />);
      expect(renderedComponent.find(TransitionGroupWrapper).prop('transform')).toContain(expected);
    });
    ...
    it('respects `scaleExtent` constraints on initial display', () => {
      ...
      let renderedComponent = shallow(<Dag data={mockData} scaleExtent={scaleExtent} zoom={0.9} />);
      expect(renderedComponent.find(TransitionGroupWrapper).prop('transform')).toContain(
        `scale(${scaleExtent.max})`
      );

      renderedComponent = shallow(<Dag data={mockData} scaleExtent={scaleExtent} zoom={0.1} />);
      expect(renderedComponent.find(TransitionGroupWrapper).prop('transform')).toContain(
  ...
  describe('Event handlers', () => {
    ...
    describe('onNodeMouseOver', () => {
      ...
      it.skip('clones the `dagNode` representation & passes it to the onNodeMouseOver callback if defined', () => {
        const onMouseOverSpy = jest.fn();
        const mockEvt = { mock: 'event' };
        const renderedComponent = mount(<Dag data={mockData} onNodeMouseOver={onMouseOverSpy} />);
    ...
    describe('onNodeMouseOut', () => {
      ...
      it('clones the `dagNode` representation & passes it to the onNodeMouseOut callback if defined', () => {
        const onMouseOutSpy = jest.fn();
        const mockEvt = { mock: 'event' };
        const renderedComponent = mount(<Dag data={mockData} onNodeMouseOut={onMouseOutSpy} />);
    ...
    describe('onUpdate', () => {
      ...
      it.skip('calls `onUpdate` on zoom', () => {
        ...
        const scrollableComponent = document.querySelector('.rd3dag-tree-container > svg');
      ...
      it.skip('does not call `onUpdate` if not a function', () => {
        ...
        document.body.innerHTML += '<div id="reactContainer"></div>';
        render(
          <Dag data={mockData} onUpdate scaleExtent={{ min: 0.1, max: 10 }} />,
          document.querySelector('#reactContainer')
        );
        const scrollableComponent = document.querySelector('.rd3dag-tree-container > svg');
        scrollableComponent.dispatchEvent(new Event('wheel'));
